...
const GroupVideoModal: FC<VideoInviteModalProps> = ({ cancle, visible, localTracks, remoteTracks }) => {
    const handerCancle = () => {
        cancle()
    }
    ...
    useEffect(() => {
        // @ts-ignore
        localTracks?.forEach((item, index) => {
            // @ts-ignore
            const ele: HTMLVideoElement = document.getElementById(`localVideo${index}`)
            console.log('流', ele, item.getType(), item, item)
            if(item.getType() === 'video') {
                ele.srcObject = item.stream
                console.log(`#localVideo${index}`)
            }
        })
        console.log('远程流', remoteTracks)
        Object.values(remoteTracks)?.forEach((item, index) => {
            // @ts-ignore
            const ele: HTMLVideoElement = document.getElementById(`remoteVideo${index}`)
            // @ts-ignore
            console.log(ele, item?.video, item?.video?.getType())
            // @ts-ignore
            if(item?.video?.getType() === 'video') {
                // @ts-ignore
                ele.srcObject = item?.video.stream
                console.log(`#localVideo${index}`)
            }
        })
    ...
    return (
        <Modal
            ...
            <div className="rtc_body" style={{ width: '858px', height: '564px' }}>
                ...
                {/* <video src="" id="localVideo1" autoPlay playsInline style={{ width: '100%', height: '100%', objectFit: 'contain' }}></video> */}
            </div>
        </Modal>
    )
}

export default GroupVideoModal
